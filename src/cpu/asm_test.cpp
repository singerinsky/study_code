...
{
    int a = 10, b = 0;
    __asm__("movl %1, %%eax;"//%1 表示输入参数对应 b
            "movl %%eax,%0;"    // %0 输出参数
            : "=r"(b)   // 输出参数,前缀 = 是限定符表示只写入 + 表示 读写
            : "r"(a)    // 输入参数
            :"%eax");   // 表示clobber list 某些汇编指令执行之后有一些副作用，可能会隐性地影响某些寄存器或者是内存的值，如果被影响的寄存器或者是内存并没有在输入、输出操作数中列出来，那么需要将这些寄存器或者内存列入clobber list中
    printf("%s:b=%d\n", __func__, b);
}

void manual_call_time_new()
{
    int time_value;
    int pass = 0;
    asm volatile("mov %0, %%ebx;"
...
TEST(TestCpu, get_address)
{
    get_new_rip();
}

TEST(TestCpu, manual_call_exit)
{
    LOG(INFO) << "start of test function" << endl;
    manual_call_time();
