...
public:
  int id = 0;
  ClassShared() { LOG(INFO) << "构造 ClassShared"; }
  ~ClassShared() { LOG(INFO) << "析构 ClassShared" << id; }
};
...
template <typename T> void template_function(T t) { LOG(INFO) << t; }

template <> void template_function<int>(int t) { LOG(INFO) << "int type"; }

TEST(BaseTest, Move) { ASSERT_EQ(eSys_Client_Show_Flag, 0x04); }
...
TEST(BaseTest, test_class_impl_function) {
  ...
  LOG(INFO) << "移动构造函数 "
            << std::is_trivially_constructible<ClassWithIntUniquePtr>::value;
  LOG(INFO) << "移动构造函数 "
            << std::is_move_constructible<ClassWithIntUniquePtr>::value;
  LOG(INFO) << "移动赋值函数 "
            << std::is_move_assignable<ClassWithIntUniquePtr>::value;
}
...
template <typename T> void test_forward(T &&value) {
  test_forward_call(std::forward<T>(value));
  // test_forward_call(value);
}

template <class T> void test_forward_call(T &&value) {
  LOG(INFO) << std::is_reference<decltype(value)>();
  LOG(INFO) << std::is_rvalue_reference<decltype(value)>();
...
TEST(BaseTest, test_class_forward) {
  ...
  pod.SetPtrValue("value set now");
  test_forward<>(std::move(pod));
  if (!pod.ptr_) {
    LOG(INFO) << "pod.ptr_ have move to other place";
  }
...
const char *test_str_1() {
  ...
  s2.append("s2");
  const char *p1 =
      s1.substr(1).data(); // substr(1)的意思是取字符串从下标1到结尾的部分
