...
void open_shared_memory_by_mmap() {
  int mmap_handler;
  // 打开或创建共享内存文件
  mmap_handler =
      open("./share_memery_handler", O_RDWR | O_CREAT | O_TRUNC, 0666);
  if (mmap_handler == -1) {
    // 文件已存在则重新打开
    LOG(INFO) << "File exist!";
  ...
  // 将共享内存文件映射到内存中
  PodSampleClass *p_object_in_shm = (PodSampleClass *)mmap(
      NULL, sizeof(PodSampleClass) * 10, PROT_READ | PROT_WRITE, MAP_SHARED,
      mmap_handler, 0);
...
void read_shared_memery_by_mmap() {
  ...
  LOG(INFO) << "FILE open rst "
            << mmap_handler; // = open("./share_memery_handler",O_RDWR);
  if (mmap_handler == -1) {
    return;
  }
...
TEST(Test_shm, test_string) {
  ...
  // 在 buff 的第 5 个位置插入字符 'c'
  buff[5] = 'c';

  // 将 const_str 复制到 buff 中，长度为 strlen(const_str)
...
TEST(Test_shm, test_object_in_vec) {
  ...
  LOG(INFO)<<"distance between begin and
  end"<<std::distance(all_object_vec.begin(),all_object_vec.end());
  */
  std::vector<int> vec = {1, 20, 3, 4, 5, 5};
  std::function<void(const std::string &)> name_call =
      [](const std::string &name) { LOG(INFO) << name; };
  std::string name = "an...";
  ...
  std::transform(vec.begin(), vec.end(), result_vec.begin(),
                 [](auto &value) -> int { return value + 1; });
  for (auto &itr : vec) {
    LOG(INFO) << itr;
  }
