...
{
    std::array<std::unique_ptr<SampleClass>, 10> class_array_test = {};
    class_array_test[0] = std::make_unique<SampleClass>();
    auto ptr = std::make_unique<SampleClass>();
    class_array_test[1] = std::forward<std::unique_ptr<SampleClass>>(
...
{
    std::array<std::shared_ptr<SampleClass>, 10> class_array_test;
    class_array_test.fill(std::make_shared<SampleClass>());
    for_each(class_array_test.begin(), class_array_test.end(), [](auto & ele)
    {
        LOG(INFO) << "will ele ref count will be " << ele.use_count();
    });
    auto& ptr = std::get<0>(class_array_test);
    for_each(class_array_test.begin(), class_array_test.end(), [](auto ele)
    {
        LOG(INFO) << "will ele ref count will be " << ele.use_count();
    });
