...
inline static void *&NEXT_OBJ(
    void *
        obj) //抢取对象头四个或者头八个字节，void*的别名，本省是内存，只能我们自己取
{
...
public:
  ...
  bool Empty() { return _list == nullptr; }
...
public:
  ...
  //算法的意图是在申请的内存越大，使用的对齐标准尺度也要变的宽松。否则不管是如何的内存大小统一使用
  //4/8字节对齐的话，会出现 8095->8096 ,8091->8092 ,8085->8088这样的内存分配大小
...
struct Span {
  ...
  void *_list =
      nullptr; //链接对象的自由链表，后面有对象就不为空，没有对象就是空
