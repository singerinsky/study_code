...
public:
    ...
    template <typename... A> CallObject(A... a)
    {
        LOG(INFO) << "call constructor " << endl;
    }

};

void for_call(int x, int y)
{
...
struct AAA
{
    using Method = std::function<R (A...)>;
};


TEST(TestStd, template_call)
{
    Compare comp;
    AAA<void, int, int>::Method newobject = for_call;
    LOG(INFO) << "start calling " << endl;
    LOG(INFO) << typeid(newobject).name() << endl;
    newobject(1, 1);
}
