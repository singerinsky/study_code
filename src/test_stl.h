...
template <class T> struct AvlTree {
  ...
  Node *root_ = nullptr;
  int CalcHeight() { return _Height(root_); }

  void Insert(Node *newNode) { insertAvl(&root_, newNode); }
...
private:
  ...
  void insertAvl(Node **currentNode, Node *newNode) {
    ...
    if (newNode->value_ > (*currentNode)->value_) {
      ...
      if (1 == _Height((*currentNode)->rightNode_) -
                   _Height((*currentNode)->leftNode_)) {
      ...
      if (1 == _Height((*currentNode)->leftNode_) -
                   _Height((*currentNode)->rightNode_)) {
...
//查找
template <class T> bool avlfind(T &value, TreeNode<T> *rootNode) {
  if (rootNode == nullptr) {
    return false;
  }
...
template <class T> void insertAvl(AvlTree<T> *tree, TreeNode<T> *newNode) {
...
void function_to_bind(int value) { LOG(INFO) << "VALUE :" << value; }

template <class T> std::enable_if_t<std::is_void<T>::value, T> test_void() {
  LOG(INFO) << " void";
}
